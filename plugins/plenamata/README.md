...
│   └── workflows/                  # → Workflows.
│       ├── plenamata.conf        # → Config for the server.
│       └── plenamata.yml         # → Actions for GitHub.
├── .tests/                         # → Tests.
│   └── php                         # → PHP tests.
...
├── plenamata.php                 # → Bootstrap plugin file.
...
### GH Actions
All steps for GH Actions you can find in `.github/workflows/plenamata.yml` file. Also, for wake up a webserver, we need to add `.github/workflows/plenamata.conf` 
...
You can disable plugin hooks very easily using a DIC. Just get the plugin object from the dependency injection container `$container_builder->get( Plenamata\Front\Front::class )`. Example just disabling frontend assets:
```
function remove_plenamata_frontend_assets( $injector ) {
    $front = $injector->make( Plenamata\Front\Front::class );
    if ( ! $front ) {
        return;
    }
...
add_action( 'plenamata_init', 'remove_plenamata_frontend_assets' );
```
...
- Each test class must extend a `PlenamataUnitTests\TestCase` class.
- You can also add some code to `PlenamataUnitTests\TestCase.php`
