...
namespace PlenamataPluginUnitTests;
...
class PluginTest extends TestCase {
	...
	public function test_run_admin() {
		expect( 'is_admin' )
			...
			->withNoArgs()
			->andReturn( true );
		$settings = \Mockery::mock( '\PlenamataPlugin\Admin\SettingsPage' );
		$settings
			->shouldReceive( 'hooks' )
			->once()
			->withNoArgs();
		$injector = \Mockery::mock( 'PlenamataPlugin\Vendor\Auryn\Injector' );
		$injector
			->shouldReceive( 'make' )
			->once()
	...
	public function test_run_front() {
		expect( 'is_admin' )
			...
			->withNoArgs()
			->andReturn( false );
		$front = \Mockery::mock( '\PlenamataPlugin\Front\Front' );
		$front
			->shouldReceive( 'hooks' )
			->once()
			->withNoArgs();
