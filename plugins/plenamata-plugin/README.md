...
You can disable plugin hooks very easily using a DIC. Just get the plugin object from the dependency injection container `$container_builder->get( PlenamataPlugin\Front\Front::class )`. Example just disabling frontend assets:
```
function remove_plenamata_plugin_frontend_assets( $injector ) {
    $front = $injector->make( PlenamataPlugin\Front\Front::class );
    if ( ! $front ) {
        return;
    }
...
add_action( 'plenamata_plugin_init', 'remove_plenamata_plugin_frontend_assets' );
```
...
- Each test class must extend a `PlenamataPluginUnitTests\TestCase` class.
- You can also add some code to `PlenamataPluginUnitTests\TestCase.php`
